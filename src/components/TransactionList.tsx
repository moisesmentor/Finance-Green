...
export const TransactionList: React.FC<TransactionListProps> = ({
  ...
  const handleDeleteItem = (tx: Transaction) => {
    if (tx.installmentGroupId && tx.installmentTotal && onDeleteGroup) {
      const choice = window.confirm(
        `Esta transação faz parte de uma compra parcelada (${tx.installmentCurrent}/${tx.installmentTotal}).\n\nClique em OK para excluir TODAS as parcelas deste parcelamento, ou Cancelar para cancelar a ação.`
      );
      if (choice) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden transition-colors">
      ...
      <div className="p-4 sm:p-5 border-b border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 transition-colors">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onAddNew}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg text-white bg-emerald-600 hover:bg-emerald-700 shadow-xs transition-colors cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2.5">
          ...
          <div className="relative lg:col-span-2">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition-colors"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
          ...
          <div>
            <select
              ...
              className="w-full px-2.5 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition-colors cursor-pointer"
            >
          ...
          <div className="flex gap-2">
            <select
              ...
              className="w-1/2 px-2 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition-colors cursor-pointer"
            >
      ...
      {filteredTransactions.length === 0 ? (
        <div className="py-16 px-4 text-center">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg text-white bg-emerald-600 hover:bg-emerald-700 transition-colors shadow-xs cursor-pointer"
          >
          ...
          {filteredTransactions.map(tx => {
            ...
            return (
              <div
                key={tx.id}
                id={`tx-row-${tx.id}`}
                className="p-3.5 sm:px-5 hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-3 group"
              >
                {/* Left Side: Icon & Details */}
                <div className="flex items-start sm:items-center gap-3 min-w-0">
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${cat.bgLight} dark:bg-slate-800 dark:border-slate-700`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {tx.installmentTotal && tx.installmentTotal > 1 && (
                        <span 
                          className="inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800"
                          title={`Compra parcelada em ${tx.installmentTotal}x`}
                        >
                      ...
                      {tx.isRecurring && (
                        <span 
                          className="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700"
      ...
      {/* Footer Subtotal */}
      {filteredTransactions.length > 0 && (
        <div className="bg-slate-50 dark:bg-slate-800/70 px-5 py-3 border-t border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-300 flex flex-col sm:flex-row sm:items-center justify-between gap-2 transition-colors">
          <div>
