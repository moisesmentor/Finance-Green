  ...
  return (
    <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      >
        ...
        <div>
          <div className="text-2xl sm:text-3xl font-bold text-slate-900 font-mono-num tracking-tight">
      ...
      >
        ...
        <div>
          <div className="text-2xl sm:text-3xl font-bold text-rose-600 font-mono-num tracking-tight">
            {formatCurrency(summary.totalExpense)}
          </div>
          <div className="mt-2 flex items-center justify-between text-xs text-slate-500 font-medium border-t border-slate-100 pt-2">
            <span className="flex items-center gap-1 text-emerald-600" title="Despesas já pagas">
              <CheckCircle2 className="w-3.5 h-3.5" />
              Pagas: {formatCurrency(summary.paidExpense)}
            </span>
            <span className="flex items-center gap-1 text-amber-600" title="Despesas pendentes">
      ...
      >
        ...
        <div>
          ...
          <div className="mt-2 text-xs text-slate-500 flex items-center justify-between">
            ...
            <span className="font-semibold text-slate-700 font-mono-num">
      ...
      >
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className="text-xs text-slate-500 hover:text-emerald-700 underline font-medium cursor-pointer"
          >
            Ajustar
          </button>
        </div>
          ...
          {summary.totalBudgetLimit > 0 ? (
            <>
              <div className="flex items-baseline justify-between">
                <div className="text-xl sm:text-2xl font-bold text-slate-900 font-mono-num tracking-tight">
                  {summary.budgetPercentageUsed.toFixed(0)}%
                  <span className="text-xs font-normal text-slate-500 ml-1">consumido</span>
                </div>
              ...
              <div className="mt-2 flex items-center justify-between text-xs text-slate-500">
                {isBudgetExceeded ? (
                  ...
                  <span>
                    Restam: <strong className="text-slate-700 font-mono-num">{formatCurrency(remainingBudget)}</strong>
                  </span>
                )}
              </div>
            </>
            ...
            <div className="py-1">
              <p className="text-xs text-slate-500 mb-2">Nenhum teto de gastos configurado para este mês.</p>
              <button
                onClick={onOpenBudgets}
